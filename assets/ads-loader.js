...
function setupTikTokEvents() {
  ...
  if (isProduct) {
    ...
    setTimeout(function () {
      ...
      window.ttq.track('ViewContent', {
        ...
        content_id:   slug,
        content_type: 'product',
        contents:     [{ content_id: slug, content_name: name || 'Product', price: price || 0, quantity: 1 }],
        currency:     'MAD',
        value:        price || 0
      });
    }, 800);
  ...
  function patchAddToCart(tries) {
    if (typeof window.addToCart === 'function' && !window._ttPatched) {
      ...
      window.addToCart = function (id, product, options, qty) {
        orig(id, product, options, qty);
        const safeId = id || 'product';
        window.ttq.track('AddToCart', {
          content_name: (product && product.name) || 'Product',
          content_id:   safeId,
          content_type: 'product',
          contents:     [{ content_id: safeId, content_name: (product && product.name) || 'Product', price: (product && product.price) || 0, quantity: Math.max(1, parseInt(qty) || 1) }],
          currency:     'MAD',
          value:        ((product && product.price) || 0) * Math.max(1, parseInt(qty) || 1),
  ...
  if (isConfirmation) {
    try {
      const ord = JSON.parse(sessionStorage.getItem('nova_confirmation_order') || 'null');
      if (ord && ord.total) {
        const safeId = ord.id || 'order';
        window.ttq.track('Purchase', {
          currency:     'MAD',
          value:        ord.total,
          content_id:   safeId,
          content_type: 'product',
          contents:     (ord.items || []).map(i => ({ content_id: i.id || safeId, content_name: i.name || 'Product', price: i.price || 0, quantity: i.quantity || 1 })),
