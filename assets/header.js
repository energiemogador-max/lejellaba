/**
 * header.js — Le Jellaba shared header
...
(function () {
  ...
  var css =
    '.site-header{position:sticky;top:0;z-index:500;background:rgba(255,255,255,0.85);backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px);border-bottom:1px solid rgba(0,0,0,.08);box-shadow:0 2px 12px rgba(0,0,0,.04);}' +
    ...
    '.nav-link.active{background:#fef2f4;color:#8B4513;font-weight:600;}' +
    ...
    '.cart-badge{position:absolute;top:2px;right:2px;background:#8B4513;color:#fff;font-size:9px;font-weight:800;border-radius:99px;min-width:15px;height:15px;display:flex;align-items:center;justify-content:center;padding:0 3px;line-height:1;}' +
  ...
  function inject() {
    ...
    function updateBadge() {
      try {
        var cart = JSON.parse(localStorage.getItem('lejellaba_cart') || '[]');
        var total = cart.reduce(function(s, i) { return s + (i.quantity || i.qty || 1); }, 0);
    ...
    window.addEventListener('lejellaba-cart-updated', updateBadge);
  }
