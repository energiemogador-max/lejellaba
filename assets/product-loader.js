...
async function hydrate() {
  // Priority 1: use inline embedded JSON — no network fetch, no redirect issues
  const inlineEl = document.getElementById('ns-product-data');
  if (inlineEl) {
    PRODUCT = JSON.parse(inlineEl.textContent);
  } else {
    // Priority 2: fetch from /api/products/ (bypasses the /products/* 301 redirect)
    const res = await fetch(`/api/products/${slug}.json`);
    if (!res.ok) throw new Error(`HTTP ${res.status} loading /api/products/${slug}.json`);
