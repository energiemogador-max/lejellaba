...
// ── Session recording (history) ──
  try{
    ...
    document.addEventListener("click", (e) => {
      const target = e.target.closest('a[href*="wa.me"], a[href*="whatsapp"], button[onclick*="waOrderClient"]');
      if (target) {
        try {
          const today = new Date().toISOString().slice(0,10);
          import("https://www.gstatic.com/firebasejs/10.8.0/firebase-database.js").then(({push})=>{
            ...
            if(geo){clickData.city=geo.city;clickData.country=geo.country;}
            push(ref(db, "wa_clicks/" + today), clickData).catch(()=>{});
          });
        } catch(err) {}
      }
    });
